...
static void init_device() {
	...
	fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;//V4L2_PIX_FMT_YVU420
	...
	//fps set
	struct v4l2_streamparm strmparam;
	strmparam.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
	if(-1 == xioctl(fd, VIDIOC_G_PARM, &strmparam))
	{
		printf("Cannot get videoc params:%s\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
	printf("videoc param numerator: %d\n", strmparam.parm.capture.timeperframe.numerator);
	printf("videoc param denominator: %d\n", strmparam.parm.capture.timeperframe.denominator);
	init_mmap();
}
...
int seq = 0;
struct timeval tmold = {0, 0};
struct timeval tmcur = {0, 0};
float  fps = 0.0;
...
char video_capture(unsigned char* dst){
	...
	if(FD_ISSET(fd, &fds)){
        ...
        if(tmold.tv_usec == 0 || (tmcur.tv_sec - tmold.tv_sec >= 5))
        {
            timedif = (tmcur.tv_sec - tmold.tv_sec) * 1000 +  \
                ((tmcur.tv_usec >= tmold.tv_usec) ? (tmcur.tv_usec - tmold.tv_usec) : (1000*1000 + tmcur.tv_usec - tmold.tv_usec)) / 1000 +\
                (tmcur.tv_usec >= tmold.tv_usec ? 0 : -1000);

            fps = (float)(seq) / (float)timedif * 1000.0;
            printf(" seq : %d, freq %f usec:%u\n",seq, (fps), timedif);
            tmold = tmcur;
            seq = 0;
        }
        else
